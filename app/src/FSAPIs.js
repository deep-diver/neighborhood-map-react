...
export const search = (near) =>
  fetch(`${api}/venues/explore?near=${near}&client_id=${client_id}&client_secret=${client_secret}`)
  .then(res => res.json())

export const getPhoto = (venueId) =>
  fetch(`${api}/venues/${venueId}/photos?&client_id=${client_id}&client_secret=${client_secret}`)
  .then(res => res.json())
